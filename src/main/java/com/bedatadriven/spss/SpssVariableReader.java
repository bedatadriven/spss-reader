...
public class SpssVariableReader {
    ...
    SpssVariableReader(SpssInputStream inputStream, int position, int logicalIndex) throws IOException {
        ...
        // but should verify...
        byte[] shortNameBytes = inputStream.readBytes(SHORT_NAME_LENGTH);
        ...
        if (missingCode > 0) {
            for (int i = 0; i < missingCode; i++) {
                if (numeric) {
                    ...
                    missingStrings.add(new String(inputStream.readBytes(SHORT_NAME_LENGTH)));
                }
            }
